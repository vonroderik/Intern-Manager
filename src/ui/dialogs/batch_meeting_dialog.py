...
class BatchMeetingDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        lbl.setStyleSheet(f"font-size: 20px; font-weight: bold; color: {COLORS['primary']};")
        ...
        self.date_edit.setStyleSheet(f"padding: 5px; background-color: {COLORS['white']};")
    ...
    def _filter_list(self):
        ...
        for intern in self.all_interns_data:
            if venue_id is None or intern.venue_id == venue_id:
                item = QListWidgetItem(f"{intern.name} (RA: {intern.registration_number or 'S/RA'})")
    ...
    def _save_batch(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Sucesso", f"{count} reuniões agendadas com sucesso!")
            self.accept()
